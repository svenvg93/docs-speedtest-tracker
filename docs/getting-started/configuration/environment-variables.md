...
| `APP_NAME` | :lucide-x:{ .required-no } | Application name in the dashboard and in notifications. | Speedtest Tracker | |
...
| Name | Required | Description | Default | Example |
| ---- | :------: | ----------- | ------- | ------- |
| `ADMIN_NAME` | :lucide-x:{ .required-no } | Name of the initial admin user. Note: Only effective during initial setup. | See [Authentication](../security/authentication.md#default-user-account) | `Speedy` |
| `ADMIN_EMAIL` | :lucide-x:{ .required-no } | Email of the initial admin user. Note: Only effective during initial setup. | | `Speedy@tracker.com` |
| `ADMIN_PASSWORD` | :lucide-x:{ .required-no } | Password of the initial admin user. Note: Only effective during initial setup. | | `superinternet` |
...
| `CHART_DATETIME_FORMAT` | :lucide-x:{ .required-no } | Set the formatting of timestamps in charts. Formatting: [PHP DateTime](https://www.php.net/manual/en/datetime.format.php) | `M. j - G:i`  | `j/m G:i` (18/10 20:06) |
| `DATETIME_FORMAT` | :lucide-x:{ .required-no } | Set the formatting of timestamps in tables and notifications. Formatting: [PHP DateTime](https://www.php.net/manual/en/datetime.format.php) | `M. j, Y g:ia` | `j M Y, G:i:s` (18 Oct 2024, 20:06:01) |
| `DISPLAY_TIMEZONE` | :lucide-x:{ .required-no } | Display timestamps in your local time. | `UTC` | `America/New_York` |
| `CONTENT_WIDTH` | :lucide-x:{ .required-no } | Width of the content section of each page.| `7xl` | See Filament [docs](https://filamentphp.com/docs/4.x/panel-configuration#customizing-the-maximum-content-width).|
| `PUBLIC_DASHBOARD` | :lucide-x:{ .required-no } | Enables the public dashboard for guest (unauthenticated) users. | `false` | `true` or `false`|
...
| Name | Required | Description | Default | Example |
| ---- | :------: | ----------- | ------- | ------- |
| `SPEEDTEST_SKIP_IPS` | :lucide-x:{ .required-no } | A comma separated list of public IP addresses where tests will be skipped when present. | | `1.1.1.1` or `1.1.0.0/24` |
| `SPEEDTEST_SCHEDULE` | :lucide-x:{ .required-no } | Cron expression used to run speedtests on a scheduled basis. [crontab.guru](https://crontab.guru/) is a helpful tool. | | `6 */2 * * *`|
...
| `SPEEDTEST_EXTERNAL_IP_URL` | :lucide-x:{ .required-no } | URL of a service used to get the external WAN IP address. | 'https://icanhazip.com' | |
| `SPEEDTEST_INTERNET_CHECK_HOSTNAME` | :lucide-x:{ .required-no } | Hostname used to ping for an active internet connection. | 'icanhazip.com' | |
